...
{
    GLuint vShader = prepareShader(GL_VERTEX_SHADER, vp);
    GLuint tcShader = prepareShader(36488, tCS);
    GLuint teShader = prepareShader(36487, tES);
    GLuint fShader = prepareShader(GL_FRAGMENT_SHADER, fp);
...
GLuint Utils::loadTexture(const char* texImagePath)
{
    ...
    if (textureRef == 0)
    {
        RAS_ERROR("didnt find texture file ");
    }
...
GLuint Utils::loadCubeMap(const char* mapDir)
{
    ...
    if (textureRef == 0)
    {
        RAS_ERROR("didnt find cube map image file");
    }
...
void Utils::printShaderLog(GLuint shader)
{
    ...
    {
        log = (char*)malloc(len);
        glGetShaderInfoLog(shader, len, &cwWriten, log);
        free(log);
        RAS_ERROR("Shader Info Log %s", log);
    }
}
...
void Utils::printProgramLog(int program)
{
    ...
    {
        log = (char*)malloc(len);
        glGetProgramInfoLog(program, len, &cWritten, log);
        free(log);
        RAS_ERROR("program Info Log %s", log);
    }
}
...
GLuint Utils::prepareShader(int shaderType, const char* shaderPath)
{
    ...
    if (shaderCompiled != 1)
    {
        ...
        RAS_ERROR("shader compilation error.");
    }
    return 0;
}
